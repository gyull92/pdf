...
const fs = require("fs");
const os = require("os"); // ✅ 추가: 사용자 홈 디렉터리용
...
// ✅ 북마크 영구 저장용 디렉터리/파일 경로 함수들
function getPersistentDataDir() {
  // 예: C:\Users\유저명\GyulPDF
  const home = os.homedir();
  const dir = path.join(home, "GyulPDF"); // 폴더 이름은 원하시는 대로 바꾸셔도 됩니다.
...
if (!gotLock) {
  ...
  app.whenReady().then(() => {
    ...
    //   - 윈도우/리눅스: 여기서 argv로 넘어옴
    //   - macOS: 대부분 open-file 이벤트로 넘어오지만 혹시 모를 경우 대비해서 한 번 더 체크
